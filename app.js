...
function parseTelegramUserSafe() {
  try {
    ...
    if (tgApp.initData) {
      ...
      if (rawUser) {
        try { return JSON.parse(rawUser); } catch (e) { console.error("initData user parse error:", e); }
      }
    }
  } catch (e) {
    console.error("parseTelegramUserSafe error:", e);
  }
  return null;
}

function fillProfileHeaderSafe() {
  try {
    const user = parseTelegramUserSafe();
    ...
    const displayId = user.id ? `ID: ${user.id}` : "ID: —";

    const nameEl = document.getElementById("clientNameTop");
    const idEl = document.getElementById("clientIdTop");
    const avatarEl = document.getElementById("clientAvatar");
    const inputEl = document.getElementById("name");

    if (nameEl) nameEl.textContent = displayName;
    if (idEl) idEl.textContent = displayId;
    if (avatarEl) avatarEl.textContent = (firstName || fullName || "C").slice(0, 1).toUpperCase();
    if (inputEl && !inputEl.value) inputEl.value = fullName;

    return true;
  } catch (e) {
    console.error("fillProfileHeaderSafe error:", e);
    return false;
  }
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  setTimeout(fillProfileHeaderSafe, 300);
  setTimeout(fillProfileHeaderSafe, 1200);
  ...
  function updateUI() {
    ...
    if (state.daily >= DAILY_LIMIT) {
      ...
      limitTextEl.textContent = "Лимит 1000 CC исчерпан. И тапать, и отправлять в копилку снова можно только через 24 часа.";
    } else {
      tapBtn.disabled = false;
      saveBtn.disabled = state.daily <= 0;
