...
document.addEventListener("DOMContentLoaded", () => {
  ...
  async function fillProfile() {
    ...
    if (user) {
      ...
      console.log("Telegram user from frontend:", user);
      return;
    }

    const backendUser = await fetchProfileFromBackend();
    if (backendUser) {
      applyProfileToUI(backendUser);
      console.log("Telegram user from backend:", backendUser);
      return;
    }
    ...
    setTimeout(async () => {
      ...
      if (delayedUser) {
        applyProfileToUI(delayedUser);
        return;
      }
      const delayedBackendUser = await fetchProfileFromBackend();
      if (delayedBackendUser) {
        applyProfileToUI(delayedBackendUser);
      }
    }, 1200);
  }
  ...
  async function sendPayload(payload) {
    ...
    if (backendUrl) {
      try {
        const response = await fetch(`${backendUrl}/api/webapp-submit`, {
          method: "POST",
