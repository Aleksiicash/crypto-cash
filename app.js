...
const STORAGE_KEY="crypto_cash_direct_bot_v7";
...
document.addEventListener("DOMContentLoaded",()=>{
  ...
  function nextResetIn(){
    if(!state.windowStart) return WINDOW_MS;
    const end=state.windowStart+WINDOW_MS;
  ...
  function updateUI(){
    ...
    if(state.daily>=DAILY_LIMIT){
      ...
      limitTextEl.textContent="Лимит 1000 CC исчерпан. Новое окно откроется через таймер выше.";
    }else{
      tapBtn.disabled=false;
      limitTextEl.textContent="Лимит: 1000 CC на каждые 24 часа. Каждый тап даёт "+REWARD_PER_TAP+" CC.";
    }
  }
